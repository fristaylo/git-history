...
const HIDDEN_COLUMNS_KEY = "git-history.hiddenColumns";

@injectable()
export class Source {
	private switchSubscriber?: (batchedCommits: IBatchedCommits) => void;
	private columnsSubscriber?: (hiddenColumns: string[]) => void;
	...
	getHiddenColumns(): string[] {
		return this.context.globalState.get<string[]>(HIDDEN_COLUMNS_KEY, []);
	}

	@link("subscription")
	async subscribeColumns(handler: (hiddenColumns: string[]) => void) {
		this.columnsSubscriber = handler;
		handler(this.getHiddenColumns());
	}

	async setHiddenColumns(hiddenColumns: string[]) {
		await this.context.globalState.update(
			HIDDEN_COLUMNS_KEY,
			hiddenColumns
		);
		this.columnsSubscriber?.(hiddenColumns);
	}
